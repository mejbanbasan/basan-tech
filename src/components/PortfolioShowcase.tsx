            ...
            >
              ...
              <div>
                <div className="relative h-60 w-full overflow-hidden bg-slate-100">
                  ...
                  <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2 z-10">
                    ...
                    {study.liveUrl && (
                      <a
                        ...
                        className="px-2.5 py-1 min-h-[32px] rounded-full bg-white/95 hover:bg-[#00976C] text-slate-800 hover:text-white backdrop-blur-md transition-all inline-flex items-center gap-1 shadow-xs text-[10px] sm:text-[11px] font-semibold shrink-0 group/live"
                  ...
                  {/* Bottom info banner */}
                  <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-xs text-slate-200 font-mono z-10">
                    <span className="font-semibold text-white truncate max-w-[70%]">{study.client}</span>
                    <span className="text-emerald-300 shrink-0 font-medium">{study.year}</span>
                  </div>
                </div>
          ...
          >
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between gap-3 sticky top-0 z-20">
              <div className="flex items-center gap-2 min-w-0">
                <span className="w-2 h-2 rounded-full bg-[#00976C] shrink-0" />
                <span className="text-xs sm:text-sm font-bold text-[#022A4E] truncate">
                  {activeModalStudy.client} — Project Details
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                {activeModalStudy.liveUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={`Visit live website for ${activeModalStudy.title}`}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 min-h-[36px] rounded-full text-xs font-semibold text-white bg-[#022A4E] hover:bg-[#00976C] transition-colors shadow-2xs"
                  >
                ...
                <button
                  ...
                  className="p-2 min-h-[36px] min-w-[36px] flex items-center justify-center rounded-full bg-white border border-slate-200 hover:bg-slate-100 text-slate-500 hover:text-slate-900 transition-colors cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <div className="p-4 sm:p-8 space-y-6 sm:space-y-8 overflow-y-auto">
              ...
              <div className="space-y-2.5">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="px-2.5 py-1 rounded-md text-[11px] font-mono bg-emerald-50 border border-emerald-200 text-emerald-800 font-semibold">
                    {activeModalStudy.categoryLabel}
                  </span>
                  <span className="px-2.5 py-1 rounded-md text-[11px] font-mono bg-slate-100 border border-slate-200 text-slate-700 font-medium">
                    {activeModalStudy.year}
                  </span>
                  <span className="text-xs text-slate-500 font-medium">
                    Client: <strong className="text-[#022A4E]">{activeModalStudy.client}</strong>
                  </span>
                </div>
              ...
              {/* Clean Image Preview (No text smeared on top of image!) */}
                ...
                {activeModalStudy.liveUrl && (
                  <div className="absolute bottom-3 right-3 sm:bottom-4 sm:right-4">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      aria-label={`Open live site for ${activeModalStudy.title}`}
                      className="px-3.5 py-2 min-h-[36px] rounded-full text-xs font-semibold bg-white/95 text-[#022A4E] hover:text-white hover:bg-[#00976C] backdrop-blur-md transition-all shrink-0 inline-flex items-center gap-1.5 shadow-md"
                    >
              ...
              <div className="flex items-center gap-2.5 w-full sm:w-auto">
                {activeModalStudy.liveUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 sm:flex-none px-4 py-2.5 rounded-full text-xs font-semibold text-slate-800 bg-white hover:bg-slate-100 border border-slate-300 transition-colors flex items-center justify-center gap-1.5 shadow-2xs"
                  >
                ...
                <button
                  ...
                  className="flex-1 sm:flex-none px-5 py-2.5 rounded-full text-xs font-bold text-white bg-[#022A4E] hover:bg-[#00976C] transition-colors flex items-center justify-center gap-1.5 shadow-xs cursor-pointer"
                >
